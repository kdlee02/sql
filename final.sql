...
create procedure check_num_checkout(
	IN bookid int,
    IN checkoutid varchar(10),
    IN stdid varchar(10),
    IN datekeep date,
    IN datech date,
    IN dateprereturn date,
    IN datereturn date,
    IN machineid varchar(10),
    IN priceoverdue int,
    IN timesextension int,
    IN return0 int)
    BEGIN
		DECLARE cnt INT;
        SELECT COUNT(*) into cnt from checkout_sys where std_id =stdid;
        IF (cnt < 3) THEN
			INSERT INTO checkout_sys(book_id,checkout_id,std_id,date_keep,date_ch, date_prereturn, date_return, machine_id,price_overdue,times_extension,return_0)
...
BEGIN
	INSERT INTO checkout_return (std_id, checkout_id, book_id)
	VALUES (new.std_id, new.checkout_id, new.book_id );
END //
DELIMITER ;
...
BEFORE UPDATE ON checkout_sys
FOR EACH ROW
BEGIN
    IF NEW.date_ch IS NOT NULL THEN
        SET NEW.date_prereturn = DATE_ADD(NEW.date_ch, INTERVAL 21 DAY);
    END IF;
END //
DELIMITER ;
...
SELECT checkout_id, date_ch, date_prereturn
FROM checkout_sys
WHERE checkout_id = 'CK12';
...
#강민애 학생이 예약한 도서제목 확인
SELECT std_name, book.title,book.book_id
...
select checkout_sys.book_id, selfs_machine.dlv_id
